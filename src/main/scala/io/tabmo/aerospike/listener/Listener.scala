...
case class AerospikeWriteListener[T]()(implicit keyConverter: AerospikeKeyConverter[T]) extends Listener[AerospikeKey[T]] with WriteListener {
  override def onSuccess(key: Key): Unit = promise.success(AerospikeKey[T](key))
  override def onFailure(exception: AerospikeException): Unit = promise.failure(new AerospikeException(exception))
}

case class AerospikeReadListener() extends Listener[AerospikeRecord] with RecordListener {
  override def onSuccess(key: Key, record: Record): Unit = promise.success(AerospikeRecord(record))
  override def onFailure(exception: AerospikeException): Unit = promise.failure(new AerospikeException(exception))
}

case class AerospikeOptionalReadListener() extends Listener[Option[AerospikeRecord]] with RecordListener {
  override def onSuccess(key: Key, record: Record): Unit = promise.success(AerospikeRecord.optional(record))
  override def onFailure(exception: AerospikeException): Unit = promise.failure(new AerospikeException(exception))
}
...
case class AerospikeReadSequenceListener[T]()(implicit keyConverter: AerospikeKeyConverter[T]) extends Listener[Map[AerospikeKey[T], AerospikeRecord]] with RecordSequenceListener {
  ...
  override def onRecord(key: Key, record: Record): Unit = stream += ((AerospikeKey(key), AerospikeRecord(record)))
  override def onSuccess(): Unit = promise.success(stream.result())
  override def onFailure(exception: AerospikeException): Unit = promise.failure(new AerospikeException(exception))
}

case class AerospikeDeleteListener() extends Listener[Boolean] with DeleteListener {
  override def onSuccess(key: Key, existed: Boolean): Unit = promise.success(existed)
  override def onFailure(exception: AerospikeException): Unit = promise.failure(new AerospikeException(exception))
}
